...
float Spreadsheet::suma(int row1, int col1, int row2, int col2)
{
	...
	for (int i = row1; i <= row2; i++)
    {
        for (int j = col1; j <= col2; j++)
        {
            ...
            { 
				if (dynamic_cast<NumericCell *>(cells[i][j]))
	                {
...
void Spreadsheet::loadFromFile(string filename)
{
    ...
    while (getline(file, line)) {
        ...
        if (cellType == "string") {
            string data;
            getline(ss, data);
            cells[0].push_back(new StringCell(data));
        } else if (cellType == "numeric") {
            float data;
            ss >> data;
            cells[0].push_back(new NumericCell(data));
        }
    }
